-- =====================================================================
-- MIGRACIONES DE LA AUDITORÍA DE SEGURIDAD
-- Ejecutar TODO este archivo de una sola vez en el SQL editor de Supabase.
--
-- ORDEN IMPORTANTE: ejecuta esto ANTES de desplegar el código nuevo.
-- La aplicación YA depende de esto; hasta que exista, fallarán:
--   · registrar un abono            -> registrar_abono()
--   · registrar un pago de interés  -> registrar_pago_interes()
--   · crear un préstamo             -> crear_prestamo_con_plan()
--   · ver el saldo disponible       -> saldo_caja()
--   · la pantalla de auditoría      -> conteo_bitacora_por_tipo()
--   · el reporte de clientes        -> clientes.score_credito
--
-- Es idempotente (create or replace / if not exists): se puede reejecutar
-- sin riesgo. No borra ni modifica ningún dato existente.
--
-- Reúne, en orden:
--   1) rpc-registrar-abono.sql       → hallazgos C2 (atomicidad) y C3 (carreras)
--   2) rendimiento-agregaciones.sql  → hallazgo M3 (consultas que no escalan)
--                                      + columnas de score que faltaban
-- =====================================================================


-- #####################################################################
-- ###  1 de 2  —  rpc-registrar-abono.sql
-- #####################################################################
...
-- #####################################################################
-- ###  2 de 2  —  rendimiento-agregaciones.sql
-- #####################################################################
